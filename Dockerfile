...
# Force HTTPS detection from Render's proxy
...
BASE_URL="https://ojs-journal-2.onrender.com"\n\
...
php -r "\$config = file_get_contents('"'"'/var/www/html/config.inc.php'"'"'); \$config = preg_replace('"'"'/^(driver\s*=\s*).*$/m'"'"', '"'"'\${1}postgres'"'"', \$config); \$config = preg_replace('"'"'/^(host\s*=\s*).*$/m'"'"', '"'"'\${1}'"'"'\"$DB_HOST\"'"'"''"'"', \$config); \$config = preg_replace('"'"'/^(username\s*=\s*).*$/m'"'"', '"'"'\${1}'"'"'\"$DB_USER\"'"'"''"'"', \$config); \$config = preg_replace('"'"'/^(password\s*=\s*).*$/m'"'"', '"'"'\${1}'"'"'\"$DB_PASS\"'"'"''"'"', \$config); \$config = preg_replace('"'"'/^(name\s*=\s*).*$/m'"'"', '"'"'\${1}'"'"'\"$DB_NAME\"'"'"''"'"', \$config); \$config = preg_replace('"'"'/^(installed\s*=\s*).*$/m'"'"', '"'"'\${1}Off'"'"', \$config); \$config = preg_replace('"'"'/^;\s*base_url\s*=.*$/m'"'"', '"'"'base_url = '"'"'\"$BASE_URL\"'"'"''"'"', \$config); \$config = preg_replace('"'"'/^base_url\s*=.*$/m'"'"', '"'"'base_url = '"'"'\"$BASE_URL\"'"'"''"'"', \$config); file_put_contents('"'"'/var/www/html/config.inc.php'"'"', \$config); echo '"'"'Config written'"'"' . PHP_EOL;"\n\
